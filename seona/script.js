// ===== DATA =====
const API_BASE = "api"; // relative path to the PHP API folder
const WHATSAPP_NUMBER = "8801706392496";
const MESSENGER_USERNAME = "joy.tmke.chudche";
const CONTACT_EMAIL = "bijoyhasan1612@gmail.com";

// BRANDS, CATEGORIES, PRODUCTS and reviews are now loaded from the database
// via loadData() below instead of being hardcoded here.
let BRANDS = [];
let CATEGORIES = [];
let PRODUCTS = [];
let reviews = [];

/* ---- old hardcoded data, replaced by API calls in loadData() ----
const _UNUSED_BRANDS = [
  {
    id: 1,
    name: "I'm From",
    slug: "im-from",
...
---- end old hardcoded data ---- */

// ===== DATA LOADING (from PHP/MySQL API) =====
async function loadData() {
  try {
    const [brandsRes, catsRes, prodsRes, reviewsRes] = await Promise.all([
      fetch(`${API_BASE}/brands.php`),
...
let currentUser = null; // logged-in user {id, name, email} or null
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Escape" && $("auth-modal").classList.contains("open")) {
    closeAuthModal();
  }
});
      ...
      <div class="cart-summary-col">
        <div class="order-summary">
          ...
          <div class="form-field">
            ...
            <input id="checkout-name" class="form-input" placeholder="Your name" value="${currentUser ? currentUser.name.replace(/"/g, "&quot;") : ""}" />
          </div>
          <div class="form-field">
            <label class="form-label">Phone Number</label>
            <input id="checkout-phone" class="form-input" placeholder="e.g. 01XXXXXXXXX" />
          </div>
          <div class="form-field" style="margin-bottom:1rem">
            <label class="form-label">Delivery Address</label>
            <textarea id="checkout-address" class="form-textarea" rows="3" placeholder="Full delivery address"></textarea>
          </div>

          <button class="btn btn-wa w-full" style="border-radius:9999px;padding:0.9rem;margin-bottom:0.75rem" id="checkout-btn" onclick="placeOrder()">
...
async function placeOrder() {
  ...
  try {
    ...
    toast(
      "Couldn't save order",
      "The order wasn't saved to the database, but you can still continue via WhatsApp.",
    );
  } finally {
    btn.disabled = false;
    btn.innerHTML = `
...
function updateAuthUI() {
  const navBtn = $("auth-nav-btn");
  const drawerLink = $("drawer-auth-link");
  const dropdown = $("account-dropdown");

  if (currentUser) {
    navBtn.textContent = currentUser.name.split(" ")[0].toUpperCase();
    if (drawerLink) drawerLink.textContent = "LOG OUT";
    $("account-dropdown-name").textContent = currentUser.name;
    $("account-dropdown-email").textContent = currentUser.email;
    const reviewNameInput = $("review-name");
    if (reviewNameInput && !reviewNameInput.value) {
      reviewNameInput.value = currentUser.name;
    }
  } else {
    navBtn.textContent = "LOGIN";
    if (drawerLink) drawerLink.textContent = "LOGIN";
...
function closeAuthModal() {
  ...
  $("auth-register-error").classList.add("hidden");
}
...
async function submitLogin(e) {
  ...
  try {
    ...
    closeAuthModal();
    e.target.reset();
    toast("Welcome back", `Logged in as ${currentUser.name}.`);
  } catch (err) {
...
async function submitReview(e) {
  ...
  try {
    const res = await fetch(`${API_BASE}/reviews.php`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ productId, reviewerName, rating, title, body }),
    });
...
async function submitContact(e) {
  ...
  try {
    await fetch(`${API_BASE}/contact.php`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, email, orderItem: order, message }),
    });
  } catch (err) {
